...
% figure(1);title(['Density with depth for model:  ', name]); hold on; plot(D(1:marker),r(1:marker)/1000,'y'); xlabel('density at P and solidus temperature [kg/m3]'); ylabel('radius, km');
...
figure(3); title(['Reference density with depth pre- and post-overturn for model:  ', name]); hold on; plot(Dsol(1:marker),r(1:marker)/1000,'y',Dsolinv,rinv/1000, 'k'); xlabel('density at 1 atm and solidus temperature [kg/m3]'); ylabel('radius, km');
% figure(4); title(['Phase densities for model:  ', name]); hold on; axis([2000 4500 5800 6400]); plot(perov, r/1000, magnesio, r/1000, bet, r/1000, gam, r/1000, maj, r/1000, stish, r/1000, cpyrox, r/1000, opyrox, r/1000, gar, r/1000, spi, r/1000, plag, r/1000, oliv, r/1000, iliq, r/1000); 
...
%figure(11); title(['log(10)planetary heat flux [W/m^2] for: ', name]); hold on; plot(time/3.1536e13, log10(flux), 'g-'); xlabel('time [Ma]'); %axis([0 1.2 0.5 4])
...
%     xlabel('chondrite-normalized ratios'); ylabel('radius, km');% legend('Source (Sm/Nd)N','Source (Lu/Hf)N');
% figure(33); title(['Trace element ratios CI chondrite normalized for overturned model:  ', name]);
%     hold on; plot((solidinv(:,6)*10^4/0.1471)./(solidinv(:,7)*10^4/0.4524), (solidinv(:,8)*10^4/0.0243)./(solidinv(:,9)*10^4/0.1040), 'rs', (liquid(:,6)*10^4/0.1471)./(liquid(:,7)*10^4/0.4524), (liquid(:,8)*10^4/0.0243)./(liquid(:,9)*10^4/0.1040), 'go'); xlabel('Source (Sm/Nd)N'); ylabel('Source (Lu/Hf)N'); axis([0.8 1.5 0.5 3.0]);
 figure(34); title(['Trace element concentrations chondrite-normalized with depth for overturned model:  ', name]);
     hold on; plot(solidinv(:,6)*10^4/0.1471, rinv/1000, solidinv(:,7)*10^4/0.4524, rinv/1000, solidinv(:,8)*10^4/0.0243, rinv/1000, solidinv(:,9)*10^4/0.1040, rinv/1000); xlabel('fraction of chondritic'); ylabel('radius, km'); legend('Sm','Nd','Lu','Hf',0); %axis([0 .5 2000 3400]);
