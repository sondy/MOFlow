function VP = BirchMurnsolidClino(Kot, Kpt, Vo, P)
...
% Pfunc2 = @(Vratio)((-1)*((3/2)*Kot*((Vratio)^(7/3) - ...
%     (Vratio)^(5/3))*(1 - (3/4)*(4 - Kpt)*((Vratio)^(2/3) - 1)) - P));
% 
% [Vpressure, FVAL, EXITFLAG] = fminbnd(Pfunc2, 0, 3);
%
% [Vpressure, FVAL, EXITFLAG] = fminbnd(@(Vratio) -Pfunc(Vratio), 0, 3);
% using anonymous functions, thanks to rlhatton
