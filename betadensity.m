function density = betadensity(Mgnum, P, T)
 
% calculates density of wadsleyite (beta olivine)
...
% calculate density from V(P,T) and molar weights of each variety of wadsleyitebeta
...
% physical parameters for (Mg, Fe) wadsleyitebeta (Mg, Fe)2(Si)1(O)4
par(1,1) = Mgnum*40.52 + (1-Mgnum)*43.15;       % Vo298, cm3/mole 
par(2,1) = Mgnum*174 + (1-Mgnum)*166;           % KT298, GPa
par(3,1) = Mgnum*4.3 + (1-Mgnum)*4.0;           % dK/dP
par(4,1) = Mgnum*(-0.0323) + (1-Mgnum)*(-0.0215); % dK/dT
par(5,1) = Mgnum*2.893e-5 + (1-Mgnum)*2.315e-5; % ao
par(6,1) = Mgnum*5.772e-9 + (1-Mgnum)*7.117e-9; % a1
par(7,1) = Mgnum*(-0.8903) + (1-Mgnum)*(-0.2430);   % a2
